...
import {ChipFactory8, IRH8GateArray as IRH8GateArray8} from "../src/ChipFactory8.sol";
import {ChipFactory9, IRH8GateArray, ILegacyFactory} from "../src/ChipFactory9.sol";
...
import {IChip8Renderer} from "../src/ChipFactory9.sol";
 ...
 */
contract ChipFactory9Test is Test {
    RH8GateArray internal gates;
    ChipFactory9 internal factory;
    ...
    function setUp() public {
        ...
        factory = new ChipFactory9(IRH8GateArray(address(gates)), address(this), ILegacyFactory(address(0)), 0);
    ...
    function test_unTickPerBloccoPerChip() public {
        ...
        factory.tick(id, 0);
        vm.expectRevert(ChipFactory9.OneTickPerBlock.selector);
        factory.tick(id, 0);
        vm.roll(block.number + 1);
        factory.tick(id, 0);
    }
    ...
    function test_tettoLiquiditaAl60() public {
        ...
        vm.prank(alice);
        vm.expectRevert(ChipFactory9.BadLiquidityShare.selector);
    ...
    /// L'aggancio v9: solo token ammessi dall'owner, riserva tirata dentro nella stessa tx.
    function _ext(string memory sym) internal returns (ChipToken ext) {
        ext = new ChipToken("Altrove", sym, 999, 1_000_000e18, address(this), 1_000_000e18, address(this));
        ext.transfer(alice, 500_000e18);
        factory.allowAttach(address(ext), true);
    }
    ...
    function test_agganciaTokenDiUnLaunchpad() public {
        ...
        ChipToken ext = _ext("ALT");
        vm.startPrank(alice);
        ext.approve(address(factory), 400_000e18);
        factory.attachToken(id, address(ext), 400_000e18, 5e18);
        vm.stopPrank();
    ...
    /// Il caso NVDA del 7 settembre: un token non ammesso non si aggancia, punto.
    function test_nienteAggancioDiTokenNonAmmessi() public {
        vm.prank(alice);
        (uint256 id, ) = factory.mint(_program(), "NAKED", "NAKED", "", 0, 0);
        ChipToken ext = new ChipToken("Altrove", "ALT0", 999, 1_000_000e18, address(this), 1_000_000e18, address(this));
        ext.transfer(alice, 10e18);
        vm.startPrank(alice);
        ext.approve(address(factory), 10e18);
        vm.expectRevert(ChipFactory9.TokenNotAllowed.selector);
        factory.attachToken(id, address(ext), 1, type(uint96).max);
        vm.stopPrank();
    }
    ...
    function test_nienteAggancioARisevaVuota() public {
        ...
        ChipToken ext = _ext("ALT2");
        vm.prank(alice);
        vm.expectRevert(ChipFactory9.ReserveNotFunded.selector);
        factory.attachToken(id, address(ext), 0, 5e18);
    }

    function test_aggancioUnaVoltaSola() public {
        (uint256 id, address token) = _mint(alice, "GIA");
        assertTrue(token != address(0));
        ChipToken ext = _ext("ALT3");
        vm.startPrank(alice);
        ext.approve(address(factory), 1e18);
        vm.expectRevert(ChipFactory9.TokenAlreadySet.selector);
        factory.attachToken(id, address(ext), 1e18, 1e18);
        vm.stopPrank();
    }
    ...
    function test_unTokenNonServeDueChip() public {
        ...
        ChipToken ext = _ext("ALT4");
        vm.startPrank(alice);
        ext.approve(address(factory), 2e18);
        factory.attachToken(a, address(ext), 1e18, 1e18);
        vm.expectRevert(abi.encodeWithSelector(ChipFactory9.TokenInUse.selector, a));
        factory.attachToken(b, address(ext), 1e18, 1e18);
        vm.stopPrank();
    }
    ...
    function test_soloIlProprietarioAggancia() public {
        ...
        ChipToken ext = _ext("ALT5");
        vm.prank(bob);
        vm.expectRevert(ChipFactory9.NotChipOwner.selector);
        factory.attachToken(id, address(ext), 1e18, 1e18);
    }

    // ---- le quattro cose imparate sul campo ----------------------------------

    function test_tickerFinoA12() public {
        assertTrue(factory.tickerAvailable("GREENCHIP"));
        assertTrue(factory.tickerAvailable("ABCDEFGHIJKL"));
        assertFalse(factory.tickerAvailable("ABCDEFGHIJKLM"));
        (uint256 id, ) = _mint(alice, "GREENCHIP");
        assertEq(factory.chipByTicker("GREENCHIP"), id);
    }

    function test_iTickerDellaVecchiaFabbricaRestanoPresi() public {
        ChipFactory8 old = new ChipFactory8(IRH8GateArray8(address(gates)), address(this));
        vm.prank(alice);
        old.mint(_program(), "OLD", "OLD", "", LIQ_BPS, TARGET);
        ChipFactory9 f9 = new ChipFactory9(IRH8GateArray(address(gates)), address(this), ILegacyFactory(address(old)), old.totalChips());
        assertEq(f9.totalChips(), 1);
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(ChipFactory9.TickerTaken.selector, 1));
        f9.mint(_program(), "OLD", "OLD", "", LIQ_BPS, TARGET);
        // gli id continuano: il primo chip della nuova e' il #2
        vm.prank(alice);
        (uint256 id, ) = f9.mint(_program(), "NEW", "NEW", "", LIQ_BPS, TARGET);
        assertEq(id, 2);
        assertEq(f9.ownerOf(2), alice);
    }

    /// Il bypass della v8: tick, restart, tick nello stesso blocco. Qui no.
    function test_restartNonRegalaUnSecondoTick() public {
        (uint256 id, ) = _mint(alice, "RST2");
        factory.tick(id, 0);
        vm.prank(alice);
        factory.restart(id);
        vm.expectRevert(ChipFactory9.OneTickPerBlock.selector);
        factory.tick(id, 0);
        vm.roll(block.number + 1);
        factory.tick(id, 0);   // il blocco dopo, come sempre
    }

    /// Riserva a zero: pausa, non morte. Quello che torna in fabbrica riaccende.
    function test_riservaVuotaMetteInPausaNonSpegne() public {
        vm.prank(alice);
        (uint256 id, ) = factory.mint(_program(), "PAUSA", "PAUSA", "", 0, 0);
        ChipToken ext = _ext("ALT6");
        vm.startPrank(alice);
        ext.approve(address(factory), 1e18);
        factory.attachToken(id, address(ext), 1e18, 1e18);   // una ricompensa sola in cassa
        vm.stopPrank();
        vm.prank(bob); factory.tick(id, 0);
        assertEq(ext.balanceOf(bob), 1e18);
        vm.roll(block.number + 1);
        vm.prank(bob); factory.tick(id, 0);                    // a zero: ReserveEmpty, ma la ricompensa resta
        (, , uint256 reward, ) = factory.emission(id);
        assertEq(reward, 1e18, "la ricompensa non deve azzerarsi");
        ext.transfer(address(factory), 3e18);                   // le fee tornano in fabbrica
        vm.roll(block.number + 1);
        vm.prank(bob); factory.tick(id, 0);
        assertEq(ext.balanceOf(bob), 2e18, "l'emissione deve ripartire");
    }
