...
contract ChipHoldersVault2Test is Test {
    ChipHoldersVault2 vault; MockNPM npm; MockWETH weth; ChipTok chipTok; Tok nvda; Tok rh4; MockFactory factory;
    ...
    function setUp() public {
        ...
        vault = new ChipHoldersVault2(INPM(address(npm)), IChipFactoryLite(address(factory)), address(rh4), address(weth),
            ISwapRouter02(address(0)), IPoolManager(address(0)), address(0), keeper, address(0xF8));
    }
    ...
    function test_collect_rejects_foreign_position() public {
        ...
        vm.expectRevert(ChipHoldersVault2.NotAChipPosition.selector);
        vault.collect(1);
    }
    ...
    function test_publish_claim_pays_pro_rata_and_anyone_can_push() public {
        ...
        vm.expectRevert(ChipHoldersVault2.AlreadyClaimed.selector);
        vault.claim(3, alice, 600e18, pa);
        ...
        vm.expectRevert(ChipHoldersVault2.BadProof.selector);
    ...
    function test_expire_returns_unclaimed_to_the_chip_pile() public {
        ...
        vault.claim(3, alice, 600e18, pa);
        vm.prank(keeper);
        vm.expectRevert(ChipHoldersVault2.NotYetExpired.selector);
        vault.expire(3);
        vm.warp(block.timestamp + 8 days);
        vm.prank(keeper);
        vault.expire(3);
        assertEq(vault.undistributed(address(chipTok), address(weth)), 0.32 ether);   // il 40% non ritirato
        vm.expectRevert(ChipHoldersVault2.BadEpoch.selector);
    ...
    function test_publish_guards() public {
        ...
        vm.startPrank(keeper);
        vm.expectRevert(ChipHoldersVault2.TooMuch.selector);
        ...
        vm.stopPrank();
        vm.prank(alice);
        vm.expectRevert(ChipHoldersVault2.NotExecutor.selector);
        vault.publish(address(chipTok), bytes32(uint256(1)), 1, a, m, 7 days);
    }
    ...
    function test_executor_named_by_factory_owner() public {
        vm.prank(alice);
        vm.expectRevert(ChipHoldersVault2.NotFactoryOwner.selector);
