...
import {IChipTokenLite} from "./ChipHoldersVault.sol";

/**
 * @title ChipBuybackVault4 — le fee che ricomprano la madre (v4: per la ChipFactory9, RH4 alla riserva della madre)
 *
 * Come il v3, con una sola differenza: la fabbrica dei chip (ChipFactory9) e il
 * posto dove atterra l'RH4 ricomprato (la ChipFactory8, dove vive la riserva
 * della madre) non sono piu' lo stesso indirizzo.
 *
 * Il v2 chiedeva alla fabbrica "questo token e' di un chip?" (chipByToken).
 * Ma la fabbrica lascia agganciare a un chip un ERC20 qualsiasi (attachToken):
 * il 7 set 2026 un chip si e' preso NVDA, e a quel punto ogni fee in NVDA
 * sarebbe finita "in riserva" (cioe' nella sua tasca), e il coniatore di
 * quel chip avrebbe incassato la meta' del creator sulle posizioni con NVDA
 * come token0. Qui un token e' un chip token solo se risponde factory() e
 * chipId() in modo coerente: un'azione resta una quota, sempre.
 *
...
contract ChipBuybackVault4 {
    ...
    address public immutable rh4Sink;        // dove va l'RH4 ricomprato: la riserva della madre (ChipFactory8)
    ...
    constructor(
        ...
        address executor_, uint256 creatorBps_, address rh4Sink_
    ) {
        rh4Sink = rh4Sink_;
    ...
    function collect(uint256 tokenId) external returns (uint256 amount0, uint256 amount1) {
        ...
        if (chipId == 0) chipId = chipOf(token1);
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        poolManager.take(rh4, rh4Sink, out);
    ...
    function _route(address token, uint256 amount, address creator) internal {
        ...
        if (amount == 0) return;
        if (chipOf(token) != 0) {
            IERC20(token).safeTransfer(address(factory), amount);   // token del chip (nato qui): riserva
        } else if (token == weth) {
