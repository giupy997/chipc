...
interface ILegacyFactory {
    function chipByTicker(bytes32 ticker) external view returns (uint256);
}
...
interface IChip8Renderer {
    function tokenURI(
        uint256 id,
        ChipFactory9.Chip calldata chip,
        string calldata logo
    ) external view returns (string memory);
}

/**
 * @title ChipFactory9 — processori a 8 bit che si possono usare (seconda fabbrica)
 *
 * Stessa macchina della ChipFactory8, con quattro cose imparate sul campo:
 *   - i ticker arrivano a 12 caratteri, e restano unici anche rispetto alla
 *     fabbrica precedente (si interroga `legacy`); gli id continuano da dove
 *     l'altra e' arrivata, cosi' un chip ha un numero solo su tutto il sito;
 *   - `restart()` non azzera il blocco dell'ultimo tick: "un ciclo per
 *     blocco" vale anche per il proprietario del chip;
 *   - una riserva a zero mette in pausa la ricompensa, non la spegne per
 *     sempre: le fee che tornano in fabbrica riaccendono l'emissione;
 *   - `attachToken` accetta solo token che l'owner della fabbrica ha
 *     ammesso, e tira la riserva dentro nella stessa transazione. Niente
 *     piu' aggancio di un'azione con un wei.
 *
 ...
 */
contract ChipFactory9 is ERC721, Ownable {
    ...
    uint256 public constant TICKER_MAX = 12;
    ...
    /// @notice La fabbrica precedente: i suoi ticker restano presi anche qui.
    ILegacyFactory public immutable legacy;
    /// @notice Token che un chip puo' agganciare (attachToken): li ammette l'owner.
    mapping(address => bool) public attachAllowed;
    ...
    event AttachAllowed(address indexed token, bool allowed);
    ...
    /// @param legacy_  la ChipFactory8 (o zero): i suoi ticker valgono anche qui
    /// @param firstId  l'ultimo id gia' usato dalla fabbrica precedente: si riparte dal successivo
    constructor(IRH8GateArray gateArray, address owner_, ILegacyFactory legacy_, uint256 firstId)
        ERC721("RH Chip", "CHIP")
        Ownable(owner_)
    {
        gates = gateArray;
    ...
    function mint(
        ...
        if (address(legacy) != address(0)) {
            uint256 takenBefore = legacy.chipByTicker(ticker);
            if (takenBefore != 0) revert TickerTaken(takenBefore);
        }
    ...
    function _reward(uint256 id, Chip storage c, uint256 cycle) internal {
        ...
        if (left == 0) {
            emit ReserveEmpty(id, cycle);   // in pausa: cio' che torna in fabbrica riaccende l'emissione
            return;
        }
    ...
    function restart(uint256 id) external {
        ...
        Chip storage c = _chips[id];
        uint256 m = c.machine;
        uint256 cycles = (m >> SHIFT_CYCLES) & MASK40;
        // stato e RAM ripartono; cicli e blocco dell'ultimo tick restano:
        // un restart non regala un secondo tick nello stesso blocco
        c.machine = (cycles << SHIFT_CYCLES) | ((m >> SHIFT_BLOCK) << SHIFT_BLOCK);
        unchecked { ++c.resets; }
     ...
     */
    function attachToken(uint256 id, address token, uint256 reserve, uint96 rewardPerCycle) external {
        _requireChipOwner(id);
        if (token == address(0) || rewardPerCycle == 0) revert NoToken();
        if (!attachAllowed[token]) revert TokenNotAllowed();
        ...
        IERC20(token).safeTransferFrom(msg.sender, address(this), reserve);   // la riserva entra qui, atomica
    ...
    /// @notice Ammette (o toglie) un token all'aggancio: la madre, un launchpad partner.
    function allowAttach(address token, bool allowed) external onlyOwner {
        attachAllowed[token] = allowed;
        emit AttachAllowed(token, allowed);
    }
